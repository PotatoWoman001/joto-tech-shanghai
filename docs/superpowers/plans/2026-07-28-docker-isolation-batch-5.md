...
- [x] 固定 SearXNG 镜像摘要并记录 arm64 验证结果。
- [x] 分别固定使用 `8888`、`8080`，且仅监听 loopback。
- [x] 分别设置 Compose 项目名、健康检查和缓存命名卷。
- [x] 启动并验证 HTML/JSON 搜索端点后停止，保留卷。
...
- [x] 修正测试依赖名称并生成两个完整 Python 锁文件。
- [x] 两个镜像固定 Python 3.12.13，安装各自依赖并运行各自测试。
- [x] 固定映射 `8765`、`8766`，分别配置健康检查。
- [x] Assistant Core 用独立命名卷保存容器内模型，不挂载宿主模型目录。
- [x] 验证 HTTP、WebSocket 回放和两组完整测试。

### Task 3: 完成记录与跨项目复核

- [x] 清理容器和网络，保留依赖/缓存/数据卷。
- [x] Git 项目只提交本批文件，保留用户未跟踪资料。
- [x] 更新中央项目目录和计划状态。
- [x] 复核所有 Compose 项目名、端口、健康检查和停止状态。
