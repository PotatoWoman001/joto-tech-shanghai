...
- [ ] 写入端口、镜像版本、非 host 网络、三个命名卷的契约测试。
- [ ] 固定 Node.js 24.18.0 和 pnpm 11.9.0，以非 root 用户运行。
- [ ] 排除宿主依赖、构建产物、测试产物和环境密钥。
...
- [ ] 创建 `web`、`api`、`db` 服务。
- [ ] Web 固定映射 `4173:4173`，API 固定映射 `4174:4174`。
- [ ] 数据库仅在项目默认网络中供 API 使用。
- [ ] 为三服务配置健康检查和依赖顺序。
- [ ] 记录启动、停止、迁移、测试和卷保留方式。
...
- [ ] 运行配置契约和 Compose 解析。
- [ ] 构建镜像并启动三服务，验证健康状态和两个宿主端口。
- [ ] 验证 API 实际使用 PostgreSQL 并完成迁移。
- [ ] 在容器中运行全部单元测试和生产构建。
- [ ] 停止容器，保留三个命名卷。
- [ ] 提交且更新中央目录。
