          ...
          {caseStudies.items.map((item, index) => (
            ...
            >
              <div className="flex items-start justify-between gap-6">
                ...
                <ArrowUpRight className="h-5 w-5 text-[#5ed29c] transition-transform duration-500 group-hover:translate-x-1 group-hover:-translate-y-1 motion-reduce:transition-none" />
              </div>
              ...
              <div className="mt-auto">
                {item.logo ? (
                  <img
                    ...
                    loading="lazy"
                    className="mb-8 h-10 w-auto max-w-[190px] object-contain object-left brightness-0 invert"
                  />
                ) : (
                ...
                <h3 className="max-w-xl text-2xl font-medium leading-tight tracking-[-0.035em] text-white sm:text-3xl">
                  {item.summary}
                </h3>
                <ul className="mt-8 flex flex-wrap gap-2" aria-label={`${item.client} capabilities`}>
                  {item.capabilities.map((capability) => (
                    <li
                      key={capability}
                      className="rounded-full border border-white/12 bg-white/[0.03] px-3 py-1.5 text-[11px] text-white/62"
                    >
                      {capability}
                    </li>
                  ))}
