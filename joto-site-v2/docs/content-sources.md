# JOTO v2 content sources

This file records the sources used for the first English homepage content layer. It is not a certification register.

## JOTO-provided source material

- The supplied `/Users/cuihua/Downloads/joto-site-main/` project was used for JOTO company, service, case-study, location and contact material.
- The approved design specification was used for the exact navigation, hero copy, solution catalogue and partnership levels.
- Partnership levels have **not** been independently verified. The first public release should confirm level wording and current validity with each vendor.
- Case-study wording has been shortened and made deliberately cautious. Client approval and precise project scope should be reviewed before public launch.
- Office addresses, global delivery coverage, phone numbers and the contact email should receive a final operational check before launch.

## Official vendor references

The following primary sources informed concise vendor descriptions. Descriptions summarize product categories only; they do not assert that JOTO holds a partnership status unless a level was explicitly supplied for this build.

## JOTO legacy website references for solution detail pages

- [JOTO legacy homepage and service scope](https://www.jototech.cn/)
- [JOTO Extreme Networks page](https://www.jototech.cn/?page_id=12703)
- [JOTO KnowBe4 page](https://www.jototech.cn/?page_id=11370)
- [JOTO OneLogin page](https://www.jototech.cn/?page_id=11177)
- [JOTO Verkada education scenario](https://www.jototech.cn/?p=16515)

The new detail pages use these references only where they directly support JOTO-specific statements. Other vendor pages describe official product categories and JOTO's general planning, integration and support services; they do not invent customer deployments or current partner levels.

### Network

- [Cisco networking](https://www.cisco.com/site/us/en/products/networking/index.html)
- [Extreme Networks products](https://www.extremenetworks.com/products)
- [HPE Aruba Networking](https://www.hpe.com/us/en/networking/hpe-aruba-networking.html)
- [Sangfor products and solutions](https://www.sangfor.com/)

### Security

- [KnowBe4 security awareness training](https://www.knowbe4.com/products/security-awareness-training)
- [Palo Alto Networks network security](https://www.paloaltonetworks.com/network-security)
- [Fortinet products](https://www.fortinet.com/products)
- [Sangfor products and solutions](https://www.sangfor.com/)
- [Check Point Quantum network security](https://www.checkpoint.com/quantum/)
- [OneLogin identity management](https://www.onelogin.com/product)

### Server & Storage

- [Dell PowerEdge servers](https://www.dell.com/en-us/shop/dell-poweredge-servers/sc/servers)
- [Huawei data storage](https://e.huawei.com/en/products/storage)
- [Inspur storage](https://en.inspur.com/en/2822489/index.html)
- [About Inspur](https://inspur.com/en/2822784/index.html)

### Collaboration

- [AudioCodes solutions and products](https://www.audiocodes.com/solutions-products)
- [About Vodia Networks](https://portal.vodia.com/en/about)
- [CyberData paging and connectivity solutions](https://www.cyberdata.net/)
- [Singlewire InformaCast](https://www.singlewire.com/informacast)

### Safeguarding

- [Verkada products](https://www.verkada.com/products/)
- [Hikvision products](https://www.hikvision.com/en/products/)
- [KEYKING integrated security solutions](https://key-king.com/)
- [KEYKING access controller example](https://keyking.net/product/397.html)

## Assets

- Existing logo files in the supplied project are reused where available.
- No third-party logo repository was used.
- KnowBe4, OneLogin, Inspur, Vodia, CyberData, InformaCast and Keyking intentionally use text fallbacks in this version because a stable, directly downloadable official logo asset was not confirmed during implementation.

### Cisco service photography

- `cisco-consulting.jpg`: [Unsplash photo 5U_28ojjgms](https://unsplash.com/photos/5U_28ojjgms), cropped responsively for the Consulting & Design card.
- `cisco-integration.jpg`: [Unsplash source image](https://images.unsplash.com/photo-1581092918056-0c4c3acd3789), cropped responsively for the Integration & Support card.
- `cisco-managed-services.jpg`: [IT technician working in a data-center server room on Pexels](https://www.pexels.com/photo/it-technician-working-in-data-center-server-room-37605911/), cropped responsively for the Managed Services card.
